...
def fetch_kma_data_month(file_url):
  try:
...
#%% 
#? fetch & merge yearly data (Jan 1st 00:00 - Dec 31st 23:00)
def get_full_year_data(year, station, api_key, base_url, save_path):
  all_data_frames = []
  
  print(f"{station}관측소 {year}년도 기상 데이터 수집중")
  ...
  for month in range(1, 13):
    ...
    if month == 2: # 윤년 제외 (epw 오류 방지)
        last_day = 28
        print(f"{year}년도는 윤년으로 2월 29일 데이터는 생략됩니다")
    day_str = str(last_day).zfill(2)
    
    #? 시작 및 종료 시간 문자열 생성 (0000시부터 2300시까지)
    starttime = f'{year}{month_str}010000'        # 1st day of every month
    endtime   = f'{year}{month_str}{day_str}2300' # last day of every month
    
    #? API url for surface meteological data
    url = base_url.format(
      starttime=starttime,
      endtime=endtime,
      station=station,
    ...
    #? Fetch every months
    monthly_df = fetch_kma_data_month(url)
    
    if not monthly_df.empty:
      all_data_frames.append(monthly_df)
      print(f"[호출완료] {year}-{month_str}")
    else:
      print(f"[호출실패] {year}-{month_str}의 정보가 누락되어있습니다")

  #? merge monthly data to one yearly dataframe
  if all_data_frames:
    print("\n=================================================")
    print("        원시 데이터 통합 및 저장        ")
    print("=================================================")
    final_df = pd.concat(all_data_frames, ignore_index=True)
    
    #? dataframe -> csv (save in local path)
...
#%%
#! Generate EPW file (ASOS data -> .epw)
def generate_epw(station_num, asos_file, base_epw, output_epw):
    
    #? 1. station_specification 
    
    station_spec = station_df[station_df['STN_ID']==station_num]
    wmo = int('47' + f'{int(station_num):03d}' + '0') 
    
    params = {
        'city'      : station_spec['STN_EN'],  # i.e., "서울"
        'region'    : station_spec['STN_EN'],  # i.e., "서울"
        'wmo'       : wmo,                      
        'latitude'  : station_spec['LAT'],     # i.e., 37.57142000
        'longitude' : station_spec['LON'],     # i.e., 126.9658000
        'timezone'  : 9.0,                     # Korean standard timezone
        'elevation' : station_spec['HT']
    }
    print(f"--- 1. EPW header info ---")
    print(f"위치: {params['city']} ({params['region']})")


    #? 2. load ASOS data and preprocessing
